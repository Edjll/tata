...
export const CleanCityMap = () => {
    ...
    const [cameras, setCameras] = useState<Camera[]>([
        {
            coordinates: [54.89218755754985, 52.2955119138831],
            icon: 1,
            id: 1
        }
    ]);
    ...
    useEffect(() => {
        RequestService
            .getInstance()
            .get(RequestService.BACKEND_URL + 'cameras')
            .then(response => {
                setCameras(
                    response.data.map(
                        (data: Camera) => {
    ...
    const [currentCamera, setCurrentCamera] = useState<Camera | null>(getCurrentCameraFromParams());
